import { guessSms, isTransfer } from '../src/app/lib/smsParser';

const cases: {
  name: string;
  sms: string;
  expect: Partial<{ payee: string; direction: string; transfer: boolean; amount: string }>;
}[] = [
  {
    ...
    expect: { payee: 'Amazon.ae', direction: 'outflow', transfer: false, amount: '156.82' },
  },
  {
    ...
    expect: { payee: 'STARZPLAY.COM', direction: 'inflow', transfer: false, amount: '4.00' },
  },
  {
    ...
    expect: { payee: 'Transfer', direction: 'inflow', transfer: true, amount: '500.00' },
  },
];
